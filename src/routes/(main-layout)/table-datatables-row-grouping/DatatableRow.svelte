<script lang="ts">
	...
	// Paginated data (after sorting and filtering)
	...
	// Recalculate grouped data when paginated data changes
	$: groupedData = Object.entries(groupBy(paginatedData, 'Office'));
...
<div class="grid gap-5">
	...
	<!-- Table -->
	<table id="employeeTable" class="table-striped table" style="width: 100%">
		<thead>
			<tr class="group dark:bg-dark-850 bg-gray-100">
				<th class="ltr:text-left rtl:text-right" on:click={() => sortTable('Name')}>Name</th>
				<th class="ltr:text-left rtl:text-right" on:click={() => sortTable('Position')}>Position</th
				>
				<th class="ltr:text-left rtl:text-right" on:click={() => sortTable('Age')}>Age</th>
				<th class="ltr:text-left rtl:text-right" on:click={() => sortTable('Start date')}
					>Start Date</th
				>
				<th class="ltr:text-left rtl:text-right" on:click={() => sortTable('Salary')}>Salary</th>
			</tr>
		</thead>
		<tbody>
			{#each groupedData as [group, employees]}
				<!-- Group Row (No "Office" column) -->
				<tr class="group dark:bg-dark-850 bg-gray-50">
					<td on:click={() => sortTable('Office')} colspan="5" class="p-3">{group}</td>
				</tr>

				<!-- Employee Rows for this group (No "Office" column) -->
