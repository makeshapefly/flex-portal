export type Invoice = {
  image: string;
  clientName: string;
  content: string;
  country: string;
  invoiceDate: string;
  dueDate: string;
  amount: string;
  status: 'Paid' | 'Unpaid' | 'Pending' | 'Overdue';
  invoicesID?: string;
};

export const invoicesListData: Invoice[] = [
  {
    image: '/assets/images/avatar/user-1.png',
    clientName: 'John Doe',
    content: 'Front End Development',
    country: 'New York',
    invoiceDate: '18 May, 2024',
    dueDate: '23 May, 2024',
    amount: '$154.98',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-2.png',
    clientName: 'Jane Smith',
    content: 'Laravel Development',
    country: 'California',
    invoiceDate: '19 May, 2024',
    dueDate: '24 May, 2024',
    amount: '$200.00',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-3.png',
    clientName: 'Alice Johnson',
    content: 'Web Designing',
    country: 'Texas',
    invoiceDate: '20 May, 2024',
    dueDate: '25 May, 2024',
    amount: '$350.75',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-4.png',
    clientName: 'Bob Brown',
    content: 'Crop top Sweater Clothing',
    country: 'Florida',
    invoiceDate: '21 May, 2024',
    dueDate: '26 May, 2024',
    amount: '$410.00',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-5.png',
    clientName: 'Charlie Davis',
    content: 'Bra Lace Crop top',
    country: 'Illinois',
    invoiceDate: '22 May, 2024',
    dueDate: '27 May, 2024',
    amount: '$100.25',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-6.png',
    clientName: 'Emily Clark',
    content: 'Asp.Net Services',
    country: 'Ohio',
    invoiceDate: '23 May, 2024',
    dueDate: '28 May, 2024',
    amount: '$250.50',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-7.png',
    clientName: 'David Martinez',
    content: 'Yellow women shoes',
    country: 'Michigan',
    invoiceDate: '24 May, 2024',
    dueDate: '29 May, 2024',
    amount: '$300.00',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-8.png',
    clientName: 'Sophia Garcia',
    content: 'Tote bag Leather Handbag Dolce',
    country: 'Georgia',
    invoiceDate: '25 May, 2024',
    dueDate: '30 May, 2024',
    amount: '$120.75',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-9.png',
    clientName: 'Liam Miller',
    content: 'Chat Application',
    country: 'North Carolina',
    invoiceDate: '26 May, 2024',
    dueDate: '31 May, 2024',
    amount: '$500.50',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-10.png',
    clientName: 'Olivia Wilson',
    content: 'Laravel Development',
    country: 'New Jersey',
    invoiceDate: '27 May, 2024',
    dueDate: '01 June, 2024',
    amount: '$450.00',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-11.png',
    clientName: 'Mason Anderson',
    content: 'Straw hat Cap Cowboy hat Sun hat',
    country: 'Virginia',
    invoiceDate: '28 May, 2024',
    dueDate: '02 June, 2024',
    amount: '$330.25',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-12.png',
    clientName: 'Ella Thomas',
    content: 'Blouse Ruffle Tube top',
    country: 'Arizona',
    invoiceDate: '29 May, 2024',
    dueDate: '03 June, 2024',
    amount: '$270.00',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-13.png',
    clientName: 'James Lee',
    content: 'Summer Dress',
    country: 'Massachusetts',
    invoiceDate: '30 May, 2024',
    dueDate: '04 June, 2024',
    amount: '$600.75',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-14.png',
    clientName: 'Isabella Harris',
    content: 'Wireless Headphones',
    country: 'Washington',
    invoiceDate: '31 May, 2024',
    dueDate: '05 June, 2024',
    amount: '$420.50',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-15.png',
    clientName: 'Michael Walker',
    content: 'Sunglasses',
    country: 'Colorado',
    invoiceDate: '01 June, 2024',
    dueDate: '06 June, 2024',
    amount: '$275.00',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-18.png',
    clientName: 'Benjamin Lopez',
    content: 'Backpack',
    country: 'California',
    invoiceDate: '03 June, 2024',
    dueDate: '08 June, 2024',
    amount: '$300.25',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-19.png',
    clientName: 'Emma Hernandez',
    content: 'Winter Coat',
    country: 'Illinois',
    invoiceDate: '04 June, 2024',
    dueDate: '09 June, 2024',
    amount: '$400.00',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-20.png',
    clientName: 'William Clark',
    content: 'Smartphone Case',
    country: 'Ohio',
    invoiceDate: '05 June, 2024',
    dueDate: '10 June, 2024',
    amount: '$200.50',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-21.png',
    clientName: 'Sophia Garcia',
    content: 'Fitness Tracker',
    country: 'Georgia',
    invoiceDate: '06 June, 2024',
    dueDate: '11 June, 2024',
    amount: '$180.75',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-22.png',
    clientName: 'Liam Miller',
    content: 'Sneakers',
    country: 'North Carolina',
    invoiceDate: '07 June, 2024',
    dueDate: '12 June, 2024',
    amount: '$500.50',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-23.png',
    clientName: 'Oliver Smith',
    content: 'Handbag',
    country: 'Washington',
    invoiceDate: '08 June, 2024',
    dueDate: '13 June, 2024',
    amount: '$350.00',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-24.png',
    clientName: 'Sophie Garcia',
    content: 'Earrings',
    country: 'Texas',
    invoiceDate: '09 June, 2024',
    dueDate: '14 June, 2024',
    amount: '$175.50',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-25.png',
    clientName: 'Noah Wilson',
    content: 'Sweater',
    country: 'New Jersey',
    invoiceDate: '10 June, 2024',
    dueDate: '15 June, 2024',
    amount: '$225.75',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-26.png',
    clientName: 'Emma Johnson',
    content: 'Cufflinks',
    country: 'California',
    invoiceDate: '11 June, 2024',
    dueDate: '16 June, 2024',
    amount: '$275.00',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-27.png',
    clientName: 'William Anderson',
    content: 'Winter Boots',
    country: 'Virginia',
    invoiceDate: '12 June, 2024',
    dueDate: '17 June, 2024',
    amount: '$200.25',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-28.png',
    clientName: 'Ava Brown',
    content: 'Beanie',
    country: 'Florida',
    invoiceDate: '13 June, 2024',
    dueDate: '18 June, 2024',
    amount: '$180.00',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-29.png',
    clientName: 'Liam Miller',
    content: 'Necklace',
    country: 'Texas',
    invoiceDate: '14 June, 2024',
    dueDate: '19 June, 2024',
    amount: '$400.50',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-30.png',
    clientName: 'Isabella Wilson',
    content: 'Sunglasses',
    country: 'California',
    invoiceDate: '15 June, 2024',
    dueDate: '20 June, 2024',
    amount: '$300.75',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-31.png',
    clientName: 'Mason Taylor',
    content: 'Watch',
    country: 'Virginia',
    invoiceDate: '16 June, 2024',
    dueDate: '21 June, 2024',
    amount: '$250.00',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-32.png',
    clientName: 'Sophia Garcia',
    content: 'T-shirt',
    country: 'Texas',
    invoiceDate: '17 June, 2024',
    dueDate: '22 June, 2024',
    amount: '$350.25',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-33.png',
    clientName: 'Olivia Martinez',
    content: 'Smartwatch',
    country: 'Michigan',
    invoiceDate: '18 June, 2024',
    dueDate: '23 June, 2024',
    amount: '$280.50',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-34.png',
    clientName: 'William Thompson',
    content: 'Web Development',
    country: 'Ohio',
    invoiceDate: '19 June, 2024',
    dueDate: '24 June, 2024',
    amount: '$190.75',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-35.png',
    clientName: 'Sophie Hernandez',
    content: 'Web Flow Chart',
    country: 'California',
    invoiceDate: '20 June, 2024',
    dueDate: '25 June, 2024',
    amount: '$410.00',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-36.png',
    clientName: 'Ethan Jackson',
    content: 'Hoodie',
    country: 'Texas',
    invoiceDate: '21 June, 2024',
    dueDate: '26 June, 2024',
    amount: '$350.25',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-37.png',
    clientName: 'Ava Wilson',
    content: 'Jeans',
    country: 'New Jersey',
    invoiceDate: '22 June, 2024',
    dueDate: '27 June, 2024',
    amount: '$220.00',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-38.png',
    clientName: 'Charlotte Garcia',
    content: 'Sweatpants',
    country: 'Florida',
    invoiceDate: '23 June, 2024',
    dueDate: '28 June, 2024',
    amount: '$180.00',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-39.png',
    clientName: 'Elijah Brown',
    content: 'Gloves',
    country: 'Illinois',
    invoiceDate: '24 June, 2024',
    dueDate: '29 June, 2024',
    amount: '$400.50',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-40.png',
    clientName: 'Ava Johnson',
    content: 'Bracelet',
    country: 'California',
    invoiceDate: '25 June, 2024',
    dueDate: '30 June, 2024',
    amount: '$300.75',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-41.png',
    clientName: 'Henry Wilson',
    content: 'Socks',
    country: 'Virginia',
    invoiceDate: '26 June, 2024',
    dueDate: '01 July, 2024',
    amount: '$250.00',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-42.png',
    clientName: 'Emma Thompson',
    content: 'Hat',
    country: 'Texas',
    invoiceDate: '27 June, 2024',
    dueDate: '02 July, 2024',
    amount: '$280.50',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-43.png',
    clientName: 'William Hernandez',
    content: 'Wallet',
    country: 'Michigan',
    invoiceDate: '28 June, 2024',
    dueDate: '03 July, 2024',
    amount: '$190.75',
    status: 'Unpaid'
  },
  {
    image: '/assets/images/avatar/user-44.png',
    clientName: 'Sophia Jackson',
    content: 'Web Designing',
    country: 'Ohio',
    invoiceDate: '29 June, 2024',
    dueDate: '04 July, 2024',
    amount: '$410.00',
    status: 'Overdue'
  },
  {
    image: '/assets/images/avatar/user-45.png',
    clientName: 'James Garcia',
    content: 'Back End Development',
    country: 'New Jersey',
    invoiceDate: '30 June, 2024',
    dueDate: '05 July, 2024',
    amount: '$350.25',
    status: 'Paid'
  },
  {
    image: '/assets/images/avatar/user-46.png',
    clientName: 'Amelia Lopez',
    content: 'Jacket',
    country: 'Florida',
    invoiceDate: '01 July, 2024',
    dueDate: '06 July, 2024',
    amount: '$220.00',
    status: 'Pending'
  },
  {
    image: '/assets/images/avatar/user-47.png',
    clientName: 'Oliver Martinez',
    content: 'Purse',
    country: 'Texas',
    invoiceDate: '02 July, 2024',
    dueDate: '07 July, 2024',
    amount: '$300.25',
    status: 'Overdue'
  }
];
