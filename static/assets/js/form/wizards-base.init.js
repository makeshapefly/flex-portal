...
Author: Srbthemes
Version: 1.0.0
File: wizards-base init Js File
*/
...
function app() {
  return {
    ...
    init() {
      this.form.imageForm = this.defaultImage;
    },
    checkPasswordStrength() {
      ...
      let value = this.form.passwords;
      if (strongRegex.test(value)) this.passwordStrengthText = 'Strong password';
      else if (mediumRegex.test(value)) this.passwordStrengthText = 'Could be stronger';
      else this.passwordStrengthText = 'Too weak';
    },
    clearErrors() {
      this.errors = {
        firstName: '',
    ...
    handleImageChange(event) {
      ...
      if (file) {
        ...
        reader.onload = () => {
          this.form.imageForm = reader.result;
          if (reader.result === this.defaultImage)
            this.errors.imageForm = 'Please upload a profile picture.';
          else this.errors.imageForm = '';
        };
        reader.readAsDataURL(file);
      } else {
        this.form.imageForm = 'path_to_default_image.jpg';
        this.errors.imageForm = '';
      }
    },
    validateStep(step) {
      this.clearErrors();
      if (step === 1) {
        ...
        if (this.form.imageForm === this.defaultImage) this.errors.imageForm = 'Image is required';
        if (!this.form.email) {
          this.errors.email = 'Email is required';
        } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.form.email)) {
          this.errors.email = 'Invalid email format';
        }
        if (!this.form.firstName || this.errors.email) return false;
      } else if (step === 2) {
        if (!this.form.passwords) this.errors.passwords = 'Password is required';
        if (!this.form.passwords || this.passwordStrengthText != 'Strong password') return false;
      } else if (step === 3) {
        if (!this.form.proffesion) this.errors.proffesion = 'Proffesion is required';
        if (!this.form.gender) this.errors.gender = 'Gender is required';
        if (!this.form.proffesion || !this.form.gender) {
          return false;
        }
