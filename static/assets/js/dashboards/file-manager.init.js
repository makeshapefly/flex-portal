...
document.addEventListener('alpine:init', () => {
  Alpine.data('overviewStorageApp', () => ({
    ...
    renderChart() {
      ...
      if (this.overviewStorageChart) this.overviewStorageChart.destroy();
...
const filesImages = import.meta.globEager('/assets/images/file-manager/icons/*.{png,jpg,jpeg,svg}');
function quickTable() {
  return {
    data: [],
    searchTrem: '',
    ...
    filterdata() {
      const searchTrem = this.searchTrem.toLowerCase();
      if (searchTrem) {
        this.data = this.originalData.filter((item) => {
          return Object.values(item).some((value) =>
            value.toString().toLowerCase().includes(searchTrem)
          );
        });
      } else {
