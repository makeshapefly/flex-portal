...
(function () {
	...
	function drawerSetting() {
		...
		allModalButtons.forEach((element) => {
			...
			if (modalId) {
				element.addEventListener('click', function () {
					...
					if (modalElement) modalElement.appendChild(backDropOverlay);
				});
			}
		});
		...
		backDropOverlay?.addEventListener('click', function () {
			if (openDrawerId) toggleElementState(openDrawerId, false, 0);
			if (openModalId) toggleElementState(openModalId, false, 200);
		});
	}
	...
	function getDatePickerOptions(item) {
		...
		if (attributes['data-minDate']) dateData.minDate = attributes['data-minDate'].value.toString();

		if (attributes['data-maxDate']) dateData.maxDate = attributes['data-maxDate'].value.toString();

		if (attributes['data-default-date'])
			dateData.defaultDate = attributes['data-default-date'].value.toString();

		if (attributes['data-multiple-date']) dateData.mode = 'multiple';

		if (attributes['data-range-date']) dateData.mode = 'range';
		...
		if (attributes['data-week-number']) dateData.weekNumbers = true;
	...
	function getTimepickerOptions(item) {
		...
		if (attributes['data-time-hrs']) timeData.time_24hr = true;
			...
			if (term) {
				this.filteredMenu = this.menu.filter(
					(item) =>
						...
						(item.children &&
							item.children.some((child) => child.title.toLowerCase().includes(term.toLowerCase())))
				);
			} else this.filteredMenu = this.menu;
			this.fetchMenu();
		},
	...
	return {
		...
		getOpen(e) {
			if (this.$data.sidebar === 'default' && this.$data.layout !== 'horizontal') {
				if (!this.$refs.button.contains(e.target)) return false;
			}
			return false;
		},
	...
	return {
		...
		updateAttributes(isResize = null) {
			...
			this.sidebar
				...
				: documentElement.removeAttribute('data-sidebar', this.sidebar);
			this.navType
		...
		setLayout(newLayout) {
			...
			if (newLayout === 'horizontal') {
				...
				this.sidebarColors =
					this.sidebarColorsOld != '' ? this.sidebarColorsOld : this.sidebarColors;
		...
		setLayoutMode(newLayoutMode) {
			...
			if (newLayoutMode !== 'dark') {
				...
				if (this.darkModeColorsOld) document.documentElement.classList.add(this.darkModeColorsOld); // manage reset time remove color class
			}
